class SubjectsController < ApplicationController
  ...
  def index
    ...
    @subjects = @subjects.joins(:comments).where("jsonb_array_length(comments.mico_data->'entities') = ?::int", params["entities_found"])               if params["entities_found"]
    @subjects = @subjects.joins(:comments).where("jsonb_array_length(comments.mico_data->'entities') <= ?::int", params["entities_found_max"])          if params["entities_found_max"]
    @subjects = @subjects.joins(:comments).where("jsonb_array_length(comments.mico_data->'entities') >= ?::int", params["entities_found_min"])          if params["entities_found_min"]
