...
# update query parts from raw form inputs
  ...
  getFilterDisplayText: (query_field, val) =>
    switch query_field
      ...
      when "vr1_daytime" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_nighttime" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_blank" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_non_blank" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_one_animal" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_simple" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_complex" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_single_species" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_only_buffalo" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_only_elephant" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_only_ostrich" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_only_warthog" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_only_wildebeest" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_only_other" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_multi_species" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_multi_including_buffalo" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_multi_including_elephant" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_multi_including_ostrich" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_multi_including_warthog" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_multi_including_wildebeest" then "in '"+query_field.substr(4)+"' test set"
      when "vr1_multi_including_none_of_the_five" then "in '"+query_field.substr(4)+"' test set"
      else query_field
